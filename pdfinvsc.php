...
$stmt = $dbh->prepare("select activos.estado,categorias.Descripcion as categoria,subcategoria.Descripcion as subcategoria ,activos.Descripcion as activo, activos.fecha_adquisicion ,activos.serial,activos.valor_adquisicion,ubicacion.Descripcion as ubicacion , sucursal.Descripcion as sucursal from activos,ubicacion,sucursal,categorias,subcategoria WHERE activos.ubicacion_idUbicacion=ubicacion.idUbicacion and activos.fecha_adquisicion BETWEEN '$Vfecha1' AND '$Vfecha2' and ubicacion.sucursal_idsucursal=sucursal.idsucursal and activos.idSubCategoria=subcategoria.idSubCategoria AND subcategoria.idCategoria=categorias.idCategoria and subcategoria.Descripcion='$referencia';");
$stmt2 = $dbh->prepare("select COUNT(*) from activos,ubicacion,sucursal,categorias,subcategoria WHERE activos.ubicacion_idUbicacion=ubicacion.idUbicacion and activos.fecha_adquisicion BETWEEN '$Vfecha1' AND '$Vfecha2' and ubicacion.sucursal_idsucursal=sucursal.idsucursal and activos.idSubCategoria=subcategoria.idSubCategoria AND subcategoria.idCategoria=categorias.idCategoria and subcategoria.Descripcion='$referencia';");
}else{
...
$stmt2 = $dbh->prepare("select COUNT(*) from activos,ubicacion,sucursal,categorias,subcategoria WHERE activos.ubicacion_idUbicacion=ubicacion.idUbicacion and ubicacion.sucursal_idsucursal=sucursal.idsucursal and activos.idSubCategoria=subcategoria.idSubCategoria AND subcategoria.idCategoria=categorias.idCategoria and subcategoria.Descripcion='$referencia';");
}

$stmt2->execute();
$data3=$stmt2->fetchAll(PDO::FETCH_ASSOC);//contador de activos por subcategoria
    ...
    <table  class="table table-striped table-bordered"  style=" font-size: 70%;">
       ...
       echo "<tr>"; 
        ...
        echo " <th align='center'>Categoria</th>";
          ...
          echo   "<th align='center'>".$row['subcategoria']."</th>";
            ...
            <th align="center">Cantidad en <?=$row['subcategoria']?></th>
            <!-- <th>Eliminar</th> -->
        </tr>
         <?php break;}?>
        ...
        foreach ($data as $row ) {
          echo "<tr>";
            echo "<td>" . $row['categoria']."</td><td>" . $row['activo'] . "</td>" .
